...
- `quote()` - Get the quote price of a token. Does not include swap fee, so it would be like the quote price that Coin Market Cap would grab. It is mainly used for calculating the price for liquidity deposit and removal, as the quote price is what you are entering those assets at.
