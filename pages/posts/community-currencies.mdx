...
We need a brand new system. As with most phenomena that bring about real change, it comes must come from bottom-up tinkering and improvement, with little to no intervention from bureaucratic organizations.

Community Currencies are the technology that will catapult a new economic system to run parallel to our current globalization system, and in the next decade it will gain so much traction, and it will come to the forefront of conversation and debate as to how we can improve the world economics.

Community Currencies are just one way that blockchains and open source tech will change how we interact with each other. But today, I believe they are the least talked about, while simultaneously having one of the highest opportunities for making lasting change. 

Why aren't they talked about? It's simple - the incentives have driven people to focus on cryptocurrencies that can grow big and grow fast. I call these "Network Currencies". Let's describe both types below: 
...
- *Community Currency* - A currency that is used by a small, well-defined group of individuals and organizations. **It is designed with a limit to its size and usage. It will sacrifice growth to obtain stability.**

As you can see, they are opposites of each other. 
...
It is worth pointing out that Network and Community Currencies can be both centralized and decentralized, and that's okay. Fully decentralized currencies have more potential, because they didn't exist before and are filling a large void. But a coin that is deliberately, and honestly centralized, is totally fine. It's more like a traditional business.
...
- Small group of people deciding how the currency should inflate, which affect millions of people. In the case of the USD, it is billions, due to it being the global reserve currency
...
We've gone through some real Community Currencies that exist right now. Let's now project into the future some ways that they will positively impact the world.
...
This is almost impossible to do in a sustainable or regenerative manner. It fails in many different places and results in some pretty bizarre behavior - [where modern countries end up shipping their recycling to poorer countries](https://www.bbc.com/news/world-48444874). On paper, the Country is allowed to write off the material as *recycled*, because the material is an output, being put into a black box of another country, where it will be "recycled". There is then no proof it actually gets recycled, and it is left up to the individual company in the poor country to decide how honest they want to be. Since we live in a global economy that runs on making as much profit as possible, you have to understand that this plastic will often not be recycled.

> TODO - Create an image for the recycling that shows the inputs, outputs, and black box. Do it like a systems thinking diagram

This is also happening because the citizens are not properly recycling the items - and often mix garbage and recycling together. Or there are items that the city cannot recycle, but the citizens were unaware of. 
...
**Local Economics enables Circular Economies to form and be tested at a small scale.** We can then slowly scale the Local Circular Economies from 1000's of people, to hundreds of thousands, to millions, as long as the technology can scale with efficient communication to match the growing size.
...
## Conclusion 
We know the global Fiat system, and monetary governing bodies around the world, are broken. They lead us from crisis to crisis, with a complete inability to stabilize the economy. Since 2008 this has been evidently clear and only continues to get worse. Bitcoin has given us a starting point, leading to many decentralized network currencies.
...
It'd be hard to predict what exactly will be successful in the Community Currency realm - but that is by design! A bottom-up system always has outputs that surprise the smartest of people. And it's exactly what we need to counter the top-down, fragile global economy we all are forced into today.
